...
xx = []
for i in range(5):
    res = requests.get(f'http://match.yuanrenxue.com/api/match/1?page={i + 1}&m={qq.replace("丨", "%E4%B8%A8")}',
                       headers=header).json()
    for _i in res['data']:
        xx.append(_i['value'])
...
print(xx)
total = 0
for ele in range(0, len(xx)):
    total = total + xx[ele]
print("列表元素之和为: ", total / 50)
