    ...
    , , function(t, e, s) {
        ...
        var a = function() {
            ...
            return a(t, [{
                ...
                value: function(t) {
                    this.state.message += "聙" + new Array(t).join("\0")
                }
            }, {
                key: "addPaddingZero",
                value: function(t) {
...
function getRandomID() {
    var t_href = 'http://www.sgcourts.gov.cn/front2/gzxxList?fjm=J20&xszt=splc';
    var H = '';
    var F = 'mozilla/5.0 (windows nt 10.0; wow64) applewebkit/537.36 (khtml, like gecko) chrome/80.0.3987.162 safari/537.36';
    var N = new Date().getTime() + "";
    var O = "" + Math.abs(getHashCode(t_href + H + F));
    var M = O.length > 2 ? 2 : O.length;
    O = O.substring(0, M);
    return N.substring(2, N.length - 3) + getRandomString(6) + O
}


function get_params(){
    var gscu = getRandomID();
    var gscs = getRandomID() + '|pv:1';
    // return {'gscu': gscu, 'gscs': gscs}
    return gscu + '||' + gscs
}
...
function strToLong(str){
    if(str){
        ...
        for(var i = 0;i < str.length;i++){
            n += (str[i].charCodeAt()) << (i % 16)
        }
        return n;
    }
}

function md2Crypt(str){
    return CryptoApi.hash("md2",str);
}

function md5Crypt(str){
    return CryptoApi.hash("md5",str);
}

function sha1Crypt(str){
    return CryptoApi.hash("sha1",str);
}

function sha256Crypt(str){
    return CryptoApi.hash("sha256",str);
}
...
function sha512Crypt(str){
    return CryptoApi.hash("sha512",str);
}


function get_sign(){
    var cookie = '{cookie}';
    // cookie = 'e98ed28dedbf4f02a4c9777782c2c9c7';
    var arry = [md2Crypt,md5Crypt,sha1Crypt,sha256Crypt,sha384Crypt,sha512Crypt];
    var index = strToLong(cookie) % arry.length;
    var sign = arry[index](cookie);
    return sign.substring(0,32);
}

console.log(get_params());
console.log(get_sign());
// 3094d3cf73fe0aa2b0edfbd6e215d04c