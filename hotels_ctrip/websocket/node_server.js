...
var node_server = ws.createServer(function(conn){
    conn.on("text", function (msg) {
        ...
        if (Object.values(cached).includes(key)){
            console.log(node_server.connections.forEach(conn=>conn.key));
            var targetConn = node_server.connections.filter(function(conn){
                return conn.key !== key;
            });
